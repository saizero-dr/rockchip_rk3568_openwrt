...
static int requestGetSignalInfo(void) {
    ...
    for (i = 0, p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next, i++) {
        ...
        if (!strcmp(rat, "NR5G-SA"))
        {
            ...
            if (err >= 13) {
                printf("CMD=SIGNALINFO,MODE=NR5G-SA,RSRQ=%d,RSSI=%d,RSRP=%d\n", nr5g_sa.RSRQ, nr5g_sa.RSSI, nr5g_sa.RSRP);
                fflush(stdout);
            }
        }
        else if (!strcmp(rat, "NR5G-NSA"))
        {
            ...
            if (err >= 8) {
                printf("CMD=SIGNALINFO,MODE=NR5G-NSA,RSRQ=%d,RSRP=%d\n", nr5g_nsa.rsrq, nr5g_nsa.rsrp);
                fflush(stdout);
            }
        ...
        else if (!strcmp(rat, "LTE"))
        {
            ...
            if (err >= 18) {
                printf("CMD=SIGNALINFO,MODE=LTE,RSRQ=%d,RSSI=%d\n", lte.rsrq, lte.rssi);
                fflush(stdout);
            }
